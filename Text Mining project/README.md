...
Hereafter, we seek a conventional way to determine the ideal epsilon, K Nearest Neighbor. By using <a href=https://www.datanovia.com/en/lessons/dbscan-density-based-clustering-essentials>knee method</a>, the optimal ε should occur before the biggest surge on 2-NN distance. Unfortunately, we cannot see where the kneecap is in the following figure. We can only tell the number is respectively 2.24 judging by perpendicular distance.
