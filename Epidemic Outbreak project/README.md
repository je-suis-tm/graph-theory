...
The objective of the system is to estimate the unknown with the known so we can make a forecast. The process of agent-based model is pretty much the same as the dynamic system except for the core of the system, θ. To obtain the value of θ, we need the other core, the probability generating function of a random graph. In this project, we propose two common configuration models, Erdős-Rényi model and Barabási-Albert model.
...
As previously stated, the mean degree is set to 120. The mean degree of Barabási-Albert model is dependent on the minimum degree. With Riemann integral, we can see that the mean degree is around two times the minimum degree. Obviously, we set the minimum degree to 60.
